...
void priv_sock_init(Session_t *sess){
    int fds[2];
    if(socketpair(PF_UNIX, SOCK_STREAM, 0, fds) == -1) //创建一对套接字
        ERR_EXIT("socketpair");
...
void priv_sock_set_proto_context(Session_t *sess){
    ...
    // activate_oobinline(sess->connfd); //开启带外数据
    // activate_signal_sigurg(sess->connfd); //开启sigurg信号
}


void priv_sock_set_nobody_context(Session_t *sess){
    close(sess->connfd); //关闭控制连接
